...
// arc, different failure mode. Fourth Vitest hop of the timeout-minutes
// lock arc after `nextjs-streaming-ai-patterns#37`,
// `agent-orchestration-platform#44`, and (in node:test form)
// `mcp-server-cookbook#49`; canonical Python first hop is
// `llm-eval-harness#63`, audit-side fingerprint is `portfolio-ops#36`
// (`audit_phase_a.py --check missing-timeout`).
//
    ...
    it(`${workflow}::${jobId} timeout is in policy band`, () => {
      ...
      expect(
        ...
        `${workflow}::${jobId} has \`timeout-minutes: ${timeout}\` ` +
          ...
          "future heavier e2e suite), bump MAX_TIMEOUT_MINUTES with a " +
